 ...
 *   3. If Pref 2 also unavailable → Smart Vacancy Fill:
 ...
 *   Each group tries:
 *   1. Pref 1 again (spots may have freed)
 *   2. Pref 2 again
 *   3. Smart vacancy fill (same block-priority order)
...
export async function runAllotment(sessionId, round) {
    ...
    for (const group of groups) {
        ...
        // --- Smart vacancy fill (BOTH rounds) ---
        // If neither preference worked, search ALL compatible rooms ordered by
        // block popularity (see BOYS_BLOCK_PRIORITY / GIRLS_BLOCK_PRIORITY),
